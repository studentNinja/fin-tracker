...
import { fetchIncomes, addIncome, deleteIncome } from '../../features/income/incomeThunks';
...
const IncomeBlock: React.FC<Props> = (props) => {
    ...
    async function handleAddIncome(title: string, number: number) {
        try {
            const result = await dispatch(addIncome({ source: title, amount: number, userId: '', date: '', createdAt: '', updatedAt: '' }));
        } catch (error) {
        ...
        <div className="block block-1">
            ...
            <div className="income-number-container">
                <div className="income-number">
                    {arrayIncome.reduce((res, curr) => res + curr.amount, 0).toLocaleString("uk-UA")}
                </div>
            ...
            <div className="list">
                {arrayIncome.map((income) => (
                    <div key={income._id} style={{ display: "flex", gap: "5px", flexDirection: "column" }}>
                        <div className="list-elem">
                            <div>{income.source}</div>
                            <div className="list-elem-end-block">
                                <div className="delete-btn">
                                    <img src={deleteBtn} onClick={() => props.showConfirmDeletePopUp(() => handleDeleteIncome(income._id))} alt="delete" />
                                </div>
