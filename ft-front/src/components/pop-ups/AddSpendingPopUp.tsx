...
const AddSpendingPopUp = (props:{cancel:()=>void, confirmAdd:(categoryId:number, title:string, number:number)=>void }) => {
    ...
    return (
        <div className="pop-up-bg" onClick={(e)=> {
            ...
            >
                ...
                <div className="input-title-pop-up">Оберіть категорію витрати
                    <select onChange={event=>setCategoryId(+event.target.value)}  className="input-pop-up" name="select">
                        {arrayCategories.map(category=><option key={category.id} value={category.id}>{category.title}</option>)
                        }
                    </select>
                {/*<input value={title}*/}
                ...
                <div className="input-title-pop-up">Введіть назву
                <input value={title}
                       onChange={event=>setTitle(event.target.value)}
                       className="input-pop-up"/></div>
                <div className="input-title-pop-up">Введіть суму
                <input min="1"
                       type="number"
                       value={number}
