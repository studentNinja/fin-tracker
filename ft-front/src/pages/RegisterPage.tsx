...
export const RegisterPage: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<IRegisterFormInput> = (data:IRegisterFormInput, event) => {
    if (event) {
      event.preventDefault(); // Запобігає стандартній поведінці форми
    }
    dispatch(registerUser({ username, email, password, initial_capital:initialCapital, saving_goal:savingGoal }));
  ...
  return  <div className="form-container">
    <div className="form-body shadow">
      <div className="form-h-holder">
        <div className="center-div form-h">{"Зареєструєтесь".toUpperCase()}</div>
        <Link to="/login" className="center-div change-option">Вже є акаунт?</Link>
      </div>
